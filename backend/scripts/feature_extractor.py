...
key = 'scckkwwc88sk8cgw4o0owoogkg8wkockw8os4c8k'
...
def deadline(timeout, *args):
    def decorate(f):
        ...
        def new_f(*args):
            # Create a timer that raises an exception after the timeout
            timer = threading.Timer(timeout, handler)
            timer.start()
            try:
                return f(*args)
            finally:
                timer.cancel()  # Ensure the timer is cancelled if the function finishes
...
def is_URL_accessible(url):
    page = None
    try:
        # Make the initial URL request
        page = requests.get(url, timeout=5)
    except:
        # Handle the case where the original URL doesn't work, try with 'www'
        parsed = urlparse(url)
    ...
    # Check the page content and status code
